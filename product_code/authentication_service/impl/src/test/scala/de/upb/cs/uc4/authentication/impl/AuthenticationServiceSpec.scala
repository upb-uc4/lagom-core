...
import de.upb.cs.uc4.shared.client.exceptions.{ UC4CriticalException, ErrorType }
  ...
  "AuthenticationService service" should {
      ...
      prepare("Test").flatMap { _ =>
        ...
        eventually(timeout(Span(20, Seconds))) {
          client.login.handleRequestHeader(addLoginHeader("Test", "Test")).invoke().failed.map { answer =>
            answer.asInstanceOf[UC4CriticalException].possibleErrorResponse.`type` should ===(ErrorType.BasicAuthorization)
          }
    ...
    "detect a wrong username" in {
      client.login.handleRequestHeader(addLoginHeader("studenta", "student")).invoke().failed.map {
        answer => answer.asInstanceOf[UC4CriticalException].possibleErrorResponse.`type` should ===(ErrorType.BasicAuthorization)
      }
    }
    ...
    "detect that a header refresh token is expired" in {
      ...
      client.refresh.handleRequestHeader(addTokenHeader(token, "refresh")).invoke().failed.map { answer =>
        answer.asInstanceOf[UC4CriticalException].possibleErrorResponse.`type` should ===(ErrorType.RefreshTokenExpired)
      }
    }
    ...
    "detect that a header refresh token is malformed" in {
      ...
      client.refresh.handleRequestHeader(addTokenHeader(token, "refresh")).invoke().failed.map { answer =>
        answer.asInstanceOf[UC4CriticalException].possibleErrorResponse.`type` should ===(ErrorType.MalformedRefreshToken)
      }
    }
    ...
    "detect that a header refresh token has a wrong signature" in {
      ...
      client.refresh.handleRequestHeader(addTokenHeader(token, "refresh")).invoke().failed.map { answer =>
        answer.asInstanceOf[UC4CriticalException].possibleErrorResponse.`type` should ===(ErrorType.RefreshTokenSignatureInvalid)
      }
    }
    ...
    "detect that a header refresh token is missing" in {
      client.refresh.invoke().failed.map { answer =>
        answer.asInstanceOf[UC4CriticalException].possibleErrorResponse.`type` should ===(ErrorType.RefreshTokenMissing)
      }
    }
