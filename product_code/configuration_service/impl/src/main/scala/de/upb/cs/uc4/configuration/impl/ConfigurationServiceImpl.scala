...
import de.upb.cs.uc4.shared.client.{ JsonVersionNumber, Utils }
...
class ConfigurationServiceImpl(override val environment: Environment)(implicit ec: ExecutionContext, config: Config)
  ...
  override def getHyperledgerNetworkVersion: ServiceCall[NotUsed, JsonVersionNumber] = ServiceCall { _ =>
    Future.successful(JsonVersionNumber(config.getString("uc4.hyperledger.network-version")))
  }
