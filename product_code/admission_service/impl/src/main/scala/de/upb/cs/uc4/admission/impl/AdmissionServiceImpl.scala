...
class AdmissionServiceImpl(
  ...
  override def getCourseAdmissions(username: Option[String], courseId: Option[String], moduleId: Option[String]): ServiceCall[NotUsed, Seq[CourseAdmission]] =
    identifiedAuthenticated(AuthenticationRole.All: _*) { (authUser, role) =>
      ServerServiceCall { (header, _) =>
        ...
        if (role != AuthenticationRole.Admin && username.isEmpty && courseId.isEmpty) {
          throw UC4Exception.NotEnoughPrivileges
        }

        if (role != AuthenticationRole.Admin && username.isDefined && username.get.trim != authUser) {
          throw UC4Exception.OwnerMismatch
        }

        // TODO : Authentication Madness

        //Check lecturer owner mismatch
        val future = {
          if (role == AuthenticationRole.Lecturer && username.isEmpty && courseId.isDefined) {
  ...
  override def getProposalAddCourseAdmission: ServiceCall[CourseAdmission, TransactionProposal] = identifiedAuthenticated(AuthenticationRole.Student) {
    (authUser, _) => {
      ServerServiceCall { (header, courseAdmissionRaw) =>
        ...
        val courseAdmission = courseAdmissionRaw.trim.copy(timestamp = LocalDateTime.now.format(DateTimeFormatter.ofPattern("YYYYMMdd_HHmmss")))
        var validationList: List[SimpleError] = List()

        //TODO : Object Validation

        certificateService.getEnrollmentId(authUser).handleRequestHeader(addAuthenticationHeader(header)).invoke().flatMap { jsonId =>
          if (courseAdmission.enrollmentId != jsonId.id) {
            throw UC4Exception.OwnerMismatch
          }

          courseService.findCourseByCourseId(courseAdmission.courseId).handleRequestHeader(addAuthenticationHeader(header)).invoke().flatMap {
            course =>
              if (!course.moduleIds.contains(courseAdmission.moduleId)) {
              ...
              matriculationService.getMatriculationData(authUser).handleRequestHeader(addAuthenticationHeader(header)).invoke().flatMap {
                data =>
                  examregService.getExaminationRegulations(Some(data.matriculationStatus.map(_.fieldOfStudy).mkString(",")), Some(true))
                    ...
                    regulations =>
                      if (!regulations.flatMap(_.modules).map(_.id).contains(courseAdmission.moduleId) && !validationList.map(_.name).contains("moduleId")) {
                      ...
                      if (validationList.nonEmpty) {
                        ...
                        entityRef.askWithStatus[Array[Byte]](replyTo => GetProposalForAddCourseAdmission(courseAdmission, replyTo)).map {
  ...
  override def getProposalDropCourseAdmission: ServiceCall[DropAdmission, TransactionProposal] = authenticated(AuthenticationRole.Student) {
    ServerServiceCall {
      (_, admission) => {
        entityRef.askWithStatus[Array[Byte]](replyTo => GetProposalForDropCourseAdmission(admission.trim, replyTo)).map {
          array => (ResponseHeader(200, MessageProtocol.empty, List()), TransactionProposal(array))
