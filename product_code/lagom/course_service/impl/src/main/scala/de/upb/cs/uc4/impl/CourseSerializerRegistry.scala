...
import de.upb.cs.uc4.impl.events.{OnCourseCreate, OnGetAllCourses}
...
/**
  ...
  * application loader.
  */
object CourseSerializerRegistry extends SharedSerializerRegistry {
  override def customSerializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    //States
    JsonSerializer[CourseState],

    //Events
    JsonSerializer[OnCourseCreate],
    JsonSerializer[OnGetAllCourses]
