...
object UserSerializerRegistry extends SharedSerializerRegistry {
  override def customSerializers: Seq[JsonSerializer[_]] = Seq(
    ...
    JsonSerializer[OnUserForceDelete],
    JsonSerializer[OnUserSoftDelete],
    JsonSerializer[OnUserUpdate],
    JsonSerializer[OnLatestMatriculationUpdate],
    //Data
    JsonSerializer[User],
    JsonSerializer[Admin],
    JsonSerializer[Lecturer],
    JsonSerializer[Student],
    JsonSerializer[Address],
    JsonSerializer[Role]
  )
}
