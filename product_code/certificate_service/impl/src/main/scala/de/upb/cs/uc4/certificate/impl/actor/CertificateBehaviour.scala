...
object CertificateBehaviour {
  ...
  /** Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */
  def create(entityContext: EntityContext[CertificateCommand]): Behavior[CertificateCommand] = {
   ...
   */
  private[impl] def create(persistenceId: PersistenceId): EventSourcedBehavior[CertificateCommand, CertificateEvent, CertificateState] = EventSourcedBehavior
